  ...
  moduleServer(id, function(input, output, session) {
    ...
    model <- eventReactive(input$baye_do, {
      bayesian_model(data(), treatment_df(), metaoutcome(),
                     outcome_measure(), model_effects(), reference_treatment())
    })

    model_sub <- eventReactive(input$sub_do, {
      bayesian_model(data_sub(), treatment_df_sub(), metaoutcome(),
                     outcome_measure(), model_effects(), filtered_reference_treatment())
    })
    ...
    output$BayesianForestPlot_sub <- renderUI({
      plotOutput(
        ...
        width="630px",
        height = BayesPixels(as.numeric(bugsnet_sumtb(bugsnetdt_sub(), metaoutcome())$Value[1]), title = TRUE)
      )
    })
    ...
    output$ref_change_bay <- renderText({
      if (!identical(filtered_reference_treatment(), reference_treatment())) {
