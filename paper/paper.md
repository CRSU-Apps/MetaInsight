...
Network meta-analysis (NMA) is a statistical method used to determine the optimal treatments for medical conditions when multiple treatment options exist.
It enables comparisons between treatments even when no individual study has compared them [@florez_2024]. 
...
NMA is widely regarded as the preferred method for comparing multiple treatments for a condition and is often conducted as part of a systematic review of the literature. 
Many NMAs are conducted by systematic review teams and/or clinical experts who may lack training in statistics or advanced evidence-synthesis methods. 
For these users, learning R and mastering the specialised packages required for NMA can constitute a significant barrier, limiting the accessibility and uptake of NMA.
...
Several R packages enable conducting NMAs including **[bnma](https://cran.r-project.org/package=bnma)**, **[gemtc](https://cran.r-project.org/package=gemtc)**, **[meta](https://cran.r-project.org/package=meta)**, **[metafor](https://cran.r-project.org/package=metafor)** and **[netmeta](https://cran.r-project.org/package=netmeta)** but they require data in different formats and use different terminology. 
The R packages that use Bayesian methods require the JAGS system dependency that may be difficult to install in controlled environments.
MetaInsight builds upon these existing packages to conduct the analysis.
...
The application is built using **[shiny](https://cran.r-project.org/package=shiny)**, the main advantage of which is that the code is written entirely in R, a language commonly used by statisticians developing methods in NMA. 
...
Fortunately, [`ExtendedTask`](https://rstudio.github.io/shiny/reference/ExtendedTask.html) in Shiny, combined with **[mirai](https://cran.r-project.org/package=mirai)** enables tasks to run asynchronously, thereby maintaining responsiveness.
...
The justification and advantages of this approach have been described elsewhere. 

## Modifications to shinyscholar

Modifications were made to the **shinyscholar** template due to the existing application containing multiple parallel analyses (up to six models), whereas **shinyscholar** was originally designed around a single model. 
...
Since MetaInsight was launched, **[multinma](https://cran.r-project.org/package=multinma)** has emerged as an alternative package for conducting Bayesian NMA, while the previous standard **gemtc** is not under active development. 
...
Specifically, [DeepSeek](https://chat.deepseek.com/) was used for simple problems, [Claude](https://claude.ai) for more complex problems and [DeepWiki](https://deepwiki.com/) for specific problems relating to R packages.

# Acknowledgements

The authors are supported by the National Institute for Health and Care Research (NIHR) Applied Research Collaboration East Midlands (ARC EM) and Leicester NIHR Biomedical Research Centre (BRC). 
The views expressed are those of the authors and not necessarily those of the NIHR or the Department of Health and Social Care. 
Wellcome funded integration with CINeMA as part of the Essential Open Source Software Programme.
