test_that("Check covariate_results function works as expected", {
  result <- covariate_results(fitted_covariate_model)
...
test_that("Check baseline_results function produces errors as expected", {
  ...
  expect_error(covariate_results("faulty_model"), "model must be an object created by baseline_model")
  expect_error(covariate_results(list(a = 1)), "model must be an object created by baseline_model")
  expect_error(covariate_results(faulty_model), "model must be an object created by baseline_model")
})
...
test_that("{shinytest2} recording: e2e_covariate_results", {
  ...
  expect_equal(stringr::str_count(stats_all, "<tr"), 8)
  ...
  # Test number of rows (including header)
  expect_equal(stringr::str_count(quant_all, "<tr"), 8)
  # Test number of columns
  expect_equal(stringr::str_count(stringr::str_extract(quant_all, "<tr>.+?</tr>"), "<th"), 6)

  text_all <- app$get_value(output = "covariate_results-all-text")
