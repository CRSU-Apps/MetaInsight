...
test_that("Check bayes_mcmc function produces errors as expected", {
  ...
  expect_error(bayes_deviance("faulty_model"), "model must be an object created by bayes_model")
  expect_error(bayes_deviance(list(a = 1)), "model must be an object created by bayes_model")
  expect_error(bayes_deviance(faulty_model), "model must be an object created by bayes_model")
})
...
test_that("{shinytest2} recording: e2e_bayes_deviance", {
  ...
  scat_all <- app$wait_for_value(output = "bayes_deviance-all-scat")
  scat_sub <- app$wait_for_value(output = "bayes_deviance-sub-scat")
  stem_all <- app$wait_for_value(output = "bayes_deviance-all-stem")
  stem_sub <- app$wait_for_value(output = "bayes_deviance-sub-stem")
  lev_all <- app$wait_for_value(output = "bayes_deviance-all-lev")
  lev_sub <- app$wait_for_value(output = "bayes_deviance-sub-lev")
