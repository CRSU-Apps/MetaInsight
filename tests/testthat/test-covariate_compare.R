test_that("Check covariate_compare function works as expected", {
  result <- covariate_compare(fitted_covariate_model)
...
test_that("Check bayes_mcmc function produces errors as expected", {
  ...
  expect_error(covariate_compare("faulty_model"), "model must be an object created by bayes_model")
  expect_error(covariate_compare(list(a = 1)), "model must be an object created by bayes_model")
  expect_error(covariate_compare(faulty_model), "model must be an object created by bayes_model")
})

test_that("{shinytest2} recording: e2e_covariate_compare", {
  ...
  table <- app$get_value(output = "covariate_compare-covariate-table")
  ...
  table_all_dl <- app$get_download("covariate_compare-covariate-download")
