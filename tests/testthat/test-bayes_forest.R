test_that("Check bayes_forest function works as expected", {
  result <- bayes_forest(fitted_bayes_model, NULL, NULL, "title")
  expect_match(result, "<svg")

  x_min_result <- bayes_forest(fitted_bayes_model, -1, NULL, "title")
  expect_match(x_min_result, "<svg")

  x_max_result <- bayes_forest(fitted_bayes_model, NULL, 1000, "title")
...
test_that("Check bayes_forest function produces errors as expected", {
  ...
  expect_error(bayes_forest(fitted_bayes_model, "1", 2, "title"), "xmin must be of class numeric")
  expect_error(bayes_forest(fitted_bayes_model, 1, "2", "title"), "xmax must be of class numeric")
  expect_error(bayes_forest(fitted_bayes_model, 1, 2, 123), "title must be of class character")

  expect_error(bayes_forest("faulty_model", 1, 2, "title"), "model must be an object created by bayes_model")
  expect_error(bayes_forest(list(a = 1), 1, 2, "title"), "model must be an object created by bayes_model")
  expect_error(bayes_forest(faulty_model, 1, 2, "title"), "model must be an object created by bayes_model")

  expect_error(bayes_forest(fitted_bayes_model, 3, 2, "title"), "xmin must be less than xmax")
  expect_error(bayes_forest(fitted_bayes_model, 3, 3, "title"), "xmin must be less than xmax")
})
