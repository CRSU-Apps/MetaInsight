test_that("Check baseline_forest function works as expected", {
  result <- baseline_forest(fitted_baseline_model, title = "title")
  expect_match(result, "<svg")

  x_min_result <- baseline_forest(fitted_baseline_model, -1, NULL, "title")
  expect_match(x_min_result, "<svg")

  x_max_result <- baseline_forest(fitted_baseline_model, NULL, 1000, "title")
...
test_that("Check baseline_forest function produces errors as expected", {
  ...
  expect_error(baseline_forest(fitted_baseline_model, "1", 2, "title"), "xmin must be of class numeric")
  expect_error(baseline_forest(fitted_baseline_model, 1, "2", "title"), "xmax must be of class numeric")
  expect_error(baseline_forest(fitted_baseline_model, 1, 2, 123), "title must be of class character")

  expect_error(baseline_forest("faulty_model", "title"), "model must be an object created by baseline_model")
  expect_error(baseline_forest(list(a = 1), "title"), "model must be an object created by baseline_model")
  expect_error(baseline_forest(faulty_model, "title"), "model must be an object created by baseline_model")

  expect_error(baseline_forest(fitted_baseline_model, 3, 2, "title"), "xmin must be less than xmax")
  expect_error(baseline_forest(fitted_baseline_model, 3, 3, "title"), "xmin must be less than xmax")
})
