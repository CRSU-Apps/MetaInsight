...
Options are provided for conducting frequentist, Bayesian, meta-regression and baseline risk meta-regression using a consistent data format and terminology.
...
If studies contain different numbers of arms, `NA` should be added.
...
For long data, covariate values must be equal for every study arm. 
...
- `ranking_option`: Whether outcome values that are less than the mean are `good` (for example blood pressure) or `bad` (for example bone density)
...
`freq_compare` produces a matrix of all treatment comparisons:

```{r freq_compare}
freq_compare(configured_data) |> knitr::kable()
```

`freq_inconsistent` summarises the direct and indirect evidence for each treatment pair and any differences between the direct and indirect evidence. 
...
```{r freq_inconsistent}
freq_inconsistent(configured_data) |> knitr::kable()
```
...
Bayesian analyses use gemtc and all the functions have the `bayes_*` prefix. To fit the model pass the configured data to `bayes_model`:
...
These models cannot be fitted to every dataset and it is not possible to determine whether it will be possible. 
The function will return an error when it is not possible.
...
As for the Bayesian models, these models are fitted with gemtc, but two parameters are required in addition to the configured data. 
...
The `regressor_type` specifies how slopes are fitted for each treatment. When `shared` the regression coefficients are the same for all treatment comparisons, when `exchangable` they differ but all come from a shared distribution and when  `unrelated` they all differ.
...
Other outputs are directly equivalent to those produced for Bayesian analyses and the functions have the same suffixes. Nodesplitting is not currently available for these models.

### Baseline risk analysis

Baseline risk quantifies differences in the initial health of participants in the studies by comparing the outcomes of study arms which imposed the reference treatment. 
These models are fitted with [bnma](https://cran.r-project.org/package=bnma) but the outputs are converted into a format consistent with gemtc. 
...
Fitting the model with `baseline_model` uses the same syntax as `covariate_model` but no covariate value is specified as this is a property of the dataset.
