  ...
  moduleServer(id, function(input, output, session) {
    observe({
      ...
      if (input$code_choice == "Module"){
        code <- readLines(system.file(glue("shiny/modules/{module}.R"), package = "metainsight"))
      }
      if (input$code_choice == "Function"){
        func <- getFromNamespace(module, ns = "metainsight")
        code <- deparse(func)
        if (length(code) == 0){
      ...
      if (input$code_choice == "Markdown"){
        if (file.exists(system.file(glue::glue("shiny/modules/{module}.Rmd"), package = "metainsight"))){
          code <- readLines(system.file(glue::glue("shiny/modules/{module}.Rmd"), package = "metainsight"))
        } else {
