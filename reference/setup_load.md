...
can be added to all formats containing covariate data where should be
replaced with the name of the covariate. For long data, covariate values
must be equal for every study arm. Risk of bias data can also be
included with all columns containing values ranging from 1 (low risk) to
3 (high risk): `rob` for the overall risk of bias, `indirectness` for
